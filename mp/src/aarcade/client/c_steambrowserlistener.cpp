...
m_RichPresenceJoinRequested(this, &C_SteamBrowserListener::RichPresenceJoinRequested),
m_NewLaunchQueryParameters(this, &C_SteamBrowserListener::NewLaunchQueryParameters)
{
...
void C_SteamBrowserListener::CreateBrowser(std::string instanceId)
{
	...
	SteamAPICall_t hAPICall = steamapicontext->SteamHTMLSurface()->CreateBrowser("", "");// "body{margin: 0; border: 4px solid #000;}");
...
void C_SteamBrowserListener::NewLaunchQueryParameters(NewLaunchQueryParameters_t* pParams)
{
	DevMsg("C_SteamBrowserListener::NewLaunchQueryParameters\n");
	std::string lobby = steamapicontext->SteamApps()->GetLaunchQueryParam("join");

	DevMsg("Lobby ID is: %s\n", lobby.c_str());

	g_pAnarchyManager->JoinLobby(lobby.c_str());
}

void C_SteamBrowserListener::RichPresenceJoinRequested(GameRichPresenceJoinRequested_t *pRequest)
{
	DevMsg("C_SteamBrowserListener::RichPresenceJoinRequested\n");

	std::string connectString = pRequest->m_rgchConnect;
	if (connectString.find("+join ") != 0)
		return;

	std::string lobby = connectString;

	size_t found = lobby.find(";");
	if (found != std::string::npos)
		lobby = lobby.substr(0, found);

	lobby = lobby.substr(6);

	//std::string lobby = connectString.substr(6);
...
void C_SteamBrowserListener::BrowserInstanceStartRequest(HTML_StartRequest_t *pResult)
{
	...
	if (!pBrowserInstance)
	{
		...
		steamapicontext->SteamHTMLSurface()->AllowStartRequest(pResult->unBrowserHandle, false);
		return;
	}
...
void C_SteamBrowserListener::BrowserInstanceURLChanged(HTML_URLChanged_t *pResult)
{
	...
	std::string uri = pResult->pchURL;
	if (uri.find("http://www.aarcadeapicall.com.net.org/?ui=") == std::string::npos)
	...
	{
		std::string uiuri = "file://";
		uiuri += engine->GetGameDirectory();
		std::replace(uiuri.begin(), uiuri.end(), '\\', '/');
		...
		uiuri += "/resource/ui/html/" + uri.substr(42);// tvdbsearch.html";

		DevMsg("Redirecting to %s\n", uiuri.c_str());

		pBrowserInstance->SetUrl(uiuri);
		//pBrowserInstance->OnBrowserInstanceURLChanged(pResult->pchURL, pResult->pchPostData, pResult->bIsRedirect, pResult->pchPageTitle, pResult->bNewNavigation);
	}
}
...
void C_SteamBrowserListener::BrowserInstanceJSAlert(HTML_JSAlert_t* pResult)
{
	if (!g_pAnarchyManager->OnSteamBrowserCallback(pResult->unBrowserHandle))
	{
		steamapicontext->SteamHTMLSurface()->JSDialogResponse(pResult->unBrowserHandle, false);
		return;
	}
