...
void C_Backpack::CreateDb()
{
	...
	std::string writeFolder = (m_workshopId == 0) ? std::string("aarcade_user/custom/") + backpackFolderName + std::string("/") : g_pAnarchyManager->GetAArcadeUserFolder() + std::string("\\consumed\\");
	std::string writeFile = (m_workshopId == 0) ? "library.db" : VarArgs("%llu.db", m_workshopId);

	std::string fullWriteFile = writeFolder + writeFile;
	if (g_pAnarchyManager->GetMetaverseManager()->CreateDb(fullWriteFile, &m_pDb))
	{
...
void C_Backpack::OpenDb()
{
	...
	std::string fullWriteFile = writeFolder + writeFile;
	g_pAnarchyManager->GetMetaverseManager()->CreateDb(fullWriteFile, &m_pDb);
}
...
void C_Backpack::LegacyAuditDb()
{
	...
	std::string typeId;
	//KeyValues* pTypeKV;
	//KeyValues* pSearchKV = new KeyValues("search");
	KeyValues* pItem;
	...
	while (pFoldername != NULL)
	{
		...
		while (pFilename != NULL)
		{
			...
			//DevMsg("loaded legacy item: %s\n", pItem->GetString("local/info/id"));

			// now save pItem to the BACKPACK'S library.db, so we don't have to scan for models next time.
			// TODO: That.
			if ( pItem)//bWasLoaded
			{
				if (!Q_strcmp(pItem->GetString("local/info/id"), ""))
				{
					DevMsg("Skipping 1 entry w/ invalid ID (model %i): %s\n", bIsModel, pItem->GetString("local/title"));
					pItem->deleteThis();
					pFilename = g_pFullFileSystem->FindNext(loadLocalItemLegacyFileHandle);
					continue;
				}

				//std::string yar = pItem->GetString("local/info/id");
				//DevMsg("Yar: %s\n", yar.c_str());
				//if (!Q_stricmp(yar.c_str(), "0c276aba"))// "31f97a3b"))// "0a8b4e1d"))
				//	DevMsg("Tester item...\n");
				...
				if (bIsModel)
				{
					...
					g_pAnarchyManager->GetMetaverseManager()->SaveSQL(&m_pDb, "models", g_pAnarchyManager->GetMetaverseManager()->GetActiveKeyValues(pItem)->GetString("info/id"), pItem);
					//pItem->deleteThis();
					modelCount++;
				}
				else
				{
					// handle types too
					typeId = g_pAnarchyManager->GetMetaverseManager()->ResolveLegacyType(g_pAnarchyManager->GetMetaverseManager()->GetActiveKeyValues(pItem)->GetString("type"), m_pDb, &m_types);
					g_pAnarchyManager->GetMetaverseManager()->GetActiveKeyValues(pItem)->SetString("type", typeId.c_str());

					// now save the item into the DB too
					...
					g_pAnarchyManager->GetMetaverseManager()->SaveSQL(&m_pDb, "items", g_pAnarchyManager->GetMetaverseManager()->GetActiveKeyValues(pItem)->GetString("info/id"), pItem);
	...
	int rc2 = sqlite3_exec(m_pDb, sqlCommitTransaction, NULL, NULL, &error);
	if (rc2 != SQLITE_OK)
	{
		DevMsg("Error executing SQLite3 statement: %s\n", sqlite3_errmsg(m_pDb));
		sqlite3_free(error);
...
void C_Backpack::MergDb()
{
	...
	while (typesIt != m_types.end())
	{
		//DevMsg("Type from DB addon is: %s (%s)\n", typesIt->first.c_str(), g_pAnarchyManager->GetMetaverseManager()->GetActiveKeyValues(typesIt->second)->GetString("title"));
...
void C_Backpack::GetAllFiles(std::vector<std::string>& allFiles)
{
	...
	rootFiles.push_back("library.db");	// TODO: Only if this is 
...
void C_Backpack::Init(std::string id, std::string title, std::string backpackFolder, uint64 workshopId, bool bDoAudit)
{
	// Initing will:
	// 1. Set basic info
	...
	std::string writeFolder = (m_workshopId == 0) ? m_backpackFolder : g_pAnarchyManager->GetAArcadeUserFolder() + std::string("\\consumed\\");
	std::string writeFile = (m_workshopId == 0) ? "library.db" : VarArgs("%llu.db", m_workshopId);

	// Make sure the folder exists
	g_pFullFileSystem->CreateDirHierarchy(writeFolder.c_str());

	// Make sure the library.db exists
	if (bDoAudit)//!g_pFullFileSystem->FileExists(VarArgs("%s%s", writeFolder.c_str(), writeFile.c_str())))
	{
		this->CreateDb();
