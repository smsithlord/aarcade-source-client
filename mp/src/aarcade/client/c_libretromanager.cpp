...
C_LibretroManager::C_LibretroManager()
{
	...
	m_pLoopLocalVideosConVar = cvar->FindVar("loop_local_videos");
	m_pSoundAllowedConVar = cvar->FindVar("libretro_sound");
...
void C_LibretroManager::DestroyLibretroInstance(C_LibretroInstance* pInstance)
{
	...
	std::string oldOriginalCore = pInstance->GetLibretroCore();
	size_t found = oldOriginalCore.find_last_of("/\\");
	if (found != std::string::npos)
		oldOriginalCore = oldOriginalCore.substr(found + 1);
	std::string oldOriginalGame = pInstance->GetOriginalGame();
	std::string oldTitle = pInstance->GetTitle();
	std::string oldId = pInstance->GetId();
	...
	if (iCurrentCount > 0 && (m_pWaitForLibretroConVar->GetBool() || bShouldReopen))
	{
		...
		if (bShouldReopen)
		{
			g_pAnarchyManager->AddToastMessage("Restarting Libretro Video");
...
bool C_LibretroManager::IsSoundAllowed()
{
	return m_pSoundAllowedConVar->GetBool();
}
