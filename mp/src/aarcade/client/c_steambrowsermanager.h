...
public:
	...
	void SetSystemCursor(unsigned long cursor, unsigned int ecursor);

	void CreateHelperInstance();

	C_SteamBrowserInstance* CreateSteamBrowserInstance();
	bool FocusSteamBrowserInstance(C_SteamBrowserInstance* pSteamBrowserInstance, bool bValue = true);
...
private:
	//std::map<unsigned int, const TCHAR*> m_cursors;
