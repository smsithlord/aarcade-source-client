...
//	DevMsg("File is: %s\n", file.c_str());
	...
	int iAAMaxString = lowerFile.length() + 1;
	char* lowerFileBuf = new char[iAAMaxString];
	Q_strncpy(lowerFileBuf, lowerFile.c_str(), iAAMaxString);
	...
	for (unsigned int i = 0; i < max; i++)
	{
		...
		int iAAMaxString = lowerPath.length() + 1;
		char* lowerPathBuf = new char[iAAMaxString];
		Q_strncpy(lowerPathBuf, lowerPath.c_str(), iAAMaxString);
