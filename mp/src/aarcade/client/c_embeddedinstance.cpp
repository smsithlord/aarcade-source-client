...
C_EmbeddedInstance::C_EmbeddedInstance()
{
	...
	m_uFrames = 0;
	m_uRenderedFrames = 0;
	m_uFramesLastSecond = 0;
	m_flLastFrameTime = 0;
	m_uFramesPerSecond = 0;
	m_flSamplerate = 48000;
	m_flFramerate = 60.0;
	m_flLastSavedSeconds = 0;
...
C_EmbeddedInstance::~C_EmbeddedInstance()
{
	...
	// NOTE: Stuff like cleaning up the animation states of entities should probably be handled here instead of in derrived classes.
}
...
void C_EmbeddedInstance::IncrementSkippedFrames()
{
	m_uFrames++;
	m_uSkippedFrames++;

	// update the frames per second, if needed
	float flTime = engine->Time();
	if (flTime > m_flLastFrameTime + 1)
	{
		// calc the FPS
		float flRange = flTime - m_flLastFrameTime;
		m_uFramesPerSecond = static_cast<int>(round(m_uFramesLastSecond / flRange));
		m_uFramesLastSecond = 0;
		m_flLastFrameTime = flTime;
	}
}

unsigned int C_EmbeddedInstance::GetFPS()
{
	return m_uFramesPerSecond;
}

void C_EmbeddedInstance::IncrementRenderedFrames()
{
	m_uFrames++;
	m_uRenderedFrames++;
	m_uFramesLastSecond++;
	...
	if (flTime > m_flLastFrameTime + 1)
	{
		...
		if (flTime > m_flLastSavedSeconds + 10)
		{
			m_flLastSavedSeconds = flTime;
