...
void CInputSlate::OnTick()
{
	...
	if ((g_pAnarchyManager->IsVRActive() && g_pAnarchyManager->IsHandTrackingActive()) || (!bFullscreen && !m_bCursorAlphaZero))
	{
		if (GetAlpha() > 0)
			SetAlpha(0);
		else
	...
	else if (((g_pAnarchyManager->IsVRActive() && g_pAnarchyManager->IsHandTrackingActive()) || bFullscreen) && m_bCursorAlphaZero)
	{
		//if (m_bCursorAlphaZero)
		//{
		...
		{
			if (g_pAnarchyManager->GetSystemCursor() != this->GetCursor())
				this->SetCursor(g_pAnarchyManager->GetSystemCursor());
		}*/
	}
