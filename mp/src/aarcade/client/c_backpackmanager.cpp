...
C_Backpack* C_BackpackManager::CreateBackpack(std::string backpackFolder, uint64 workshopId, bool bDoAudit)
{
	C_Backpack* pBackpack = new C_Backpack();
	pBackpack->Init("", "", backpackFolder, workshopId, bDoAudit);
...
std::string C_BackpackManager::ExtractBackpackFoldeNameFromPath(std::string backpackFolder)
{
	...
	*/
	...
	std::string testBackpackFolder = backpackFolder;
	std::transform(testBackpackFolder.begin(), testBackpackFolder.end(), testBackpackFolder.begin(), ::tolower);
	std::replace(testBackpackFolder.begin(), testBackpackFolder.end(), '/', '\\'); // replace all '/' to '\'

	std::string testCustomFolder = m_customFolder;
	std::transform(testCustomFolder.begin(), testCustomFolder.end(), testCustomFolder.begin(), ::tolower);
	std::replace(testCustomFolder.begin(), testCustomFolder.end(), '/', '\\'); // replace all '/' to '\'

	std::string folderName = backpackFolder;
	if (testBackpackFolder.find(testCustomFolder) == 0)
	{
		folderName = backpackFolder.substr(m_customFolder.length());
...
{
	// FIXME: WRONG.  This code assumes the backpack lives in the m_customFolder
