...
void CViewRender::SetUpViews()
{
	...
	if ( UseVR() )
	{
		...
		//g_ClientVirtualReality.OverrideStereoView( &m_View, &m_ViewLeft, &m_ViewRight );	// Added for Anarchy Arcade
	}
	else
	{
		// left and right stereo views should default to being the same as the mono/middle view
...
for (StereoEye_t eEye = GetLastEye(); eEye >= GetFirstEye(); eEye = (StereoEye_t)(eEye - 1))
{
	...
	if (g_pHLVR->ShouldRunInVR())
		view.m_flAspectRatio = (float)view.width / (float)view.height;
	else if (view.m_flAspectRatio <= 0.f) // if we still don't have an aspect ratio, compute it from the view size
		view.m_flAspectRatio = (float)view.width / (float)view.height;
...
// End added for Anarchy Arcade
	...
	if( false && !g_ClientVirtualReality.ShouldRenderHUDInWorld() )//g_pHLVR->ShouldRunInSBS() && 
	{
		CViewSetup view2d;
		view2d.x				= rect->x;
		view2d.y				= rect->y;
