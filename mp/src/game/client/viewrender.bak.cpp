...
#define GUI_BACK_BUFFER "_rt_vgui"
...
static ConVar hlvr_scopefov("hlvr_scopefov", "1", FCVAR_ARCHIVE);
...
static ConVar hlvr_hudoffset("hlvr_hudoffset", "0", FCVAR_ARCHIVE);
...
extern ConVar vr_render_hud_in_world;
...
bool CViewRender::ShouldDrawViewModel( bool bDrawViewmodel )
{
	...
	//if ( !r_drawviewmodel.GetBool() )	// Added for Anarchy Arcade
	//	return false;
...
void CViewRender::DrawViewModels( const CViewSetup &view, bool drawViewmodel )
{
	...
	{
		pRTColor = g_pHLVR->GetRenderTarget( (ISourceVirtualReality::VREye) (view.m_eStereoEye - 1), ISourceVirtualReality::RT_Color );
	}

	render->Push3DView( viewModelSetup, 0, pRTColor, GetFrustum());
...
// Purpose: Performs screen space effects, if any
//-----------------------------------------------------------------------------
void CViewRender::PerformScreenOverlay(const CViewSetup& viewSetup, int x, int y, int w, int h )
{
	...
	if (m_ScreenOverlayMaterial)
	{
		...
		else
		{
			...
			ViewDrawFade(viewSetup, color, m_ScreenOverlayMaterial );
		}
	}
}
...
void CViewRender::SetupMain3DView( const CViewSetup &view, int &nClearFlags )
{
	...
	{*/
		...
		if( view.m_eStereoEye != STEREO_EYE_MONO )
		{
			pRT = g_pHLVR->GetRenderTarget( (ISourceVirtualReality::VREye) view.m_eStereoEye, ISourceVirtualReality::RT_Color );
		}

		render->Push3DView(view, nClearFlags, pRT, GetFrustum());
	//}
...
//-----------------------------------------------------------------------------
// Purpose: HLVR: This copies the left or right eye image to the mono image so as to not render a 3rd time
// Input  : &view -
//			eyeToCopy -
//-----------------------------------------------------------------------------
void CViewRender::RenderVREyeToView(const CViewSetup &view, StereoEye_t eyeToCopy, bool isMenuUp)
{
	if (eyeToCopy != STEREO_EYE_LEFT && eyeToCopy != STEREO_EYE_RIGHT)
		return;

	CMatRenderContextPtr pRenderContext(materials);

	ITexture* pVRFullFrame = materials->FindTexture(isMenuUp ? "_rt_menu" : VR_BACK_BUFFER, TEXTURE_GROUP_RENDER_TARGET);

	ITexture* pMainFullFrame = materials->FindTexture(FULL_MAIN_FB, TEXTURE_GROUP_RENDER_TARGET);
	...
	SourceRect.width = pVRFullFrame->GetActualWidth() / (isMenuUp ? 1 : 2);
	SourceRect.height = isMenuUp ? pVRFullFrame->GetActualHeight() : SourceRect.width * DestinationRect.height / DestinationRect.width;
	SourceRect.x = isMenuUp ? 0 : (eyeToCopy == STEREO_EYE_LEFT ? 0 : pVRFullFrame->GetActualWidth() / 2);
	SourceRect.y = isMenuUp ? 0 : (pVRFullFrame->GetActualHeight() / 2) - (SourceRect.height / 2);

	pRenderContext->CopyTextureToRenderTargetEx(0, pVRFullFrame, &SourceRect, &DestinationRect);
}

// HLVR !!!
// This code is from Valve's leaked sources. If we don't want to get in trouble, we could rewrite it differently ?
void CViewRender::ViewDrawFade(const CViewSetup& view, byte *color, IMaterial* pFadeMaterial)
{
	if (!color || !color[3])
		return;
...
void CViewRender::RenderView(const CViewSetup &view, int nClearFlags, int whatToDraw)
{
	if (view.m_eStereoEye == STEREO_EYE_MONO)
	{
		...
		{
			RenderVREyeToView(view, STEREO_EYE_RIGHT, (enginevgui && enginevgui->IsGameUIVisible()));
			return;
		}
	...
	else
	{
		...
		ViewDrawFade(view , color, pMaterial );
	
		PerformScreenOverlay(view, view.x, view.y, view.width, view.height );
		...
		if ( !building_cubemaps.GetBool() && view.m_bDoBloomAndToneMapping )
		{
			pRenderContext.GetFrom( materials );
			{
				...
				DoEnginePostProcessing( view.x, view.y, view.width, view.height, bFlashlightIsOn, view.m_eStereoEye );
			}
			pRenderContext.SafeRelease();
		}
	...
	if( UseVR() )
	{
		saveRenderTarget = g_pHLVR->GetRenderTarget( (ISourceVirtualReality::VREye)(view.m_eStereoEye), ISourceVirtualReality::RT_Color );
	}
	...
	if ( whatToDraw & RENDERVIEW_DRAWHUD )
	{
		...
		if( UseVR() )
		{	
			pTexture = materials->FindTexture( "_rt_vgui", NULL, false );
			bPaintMainMenu = true;
			bClear = true;
			viewX = 0;
			viewY = 0;
		...
		if ( root != 0 )
		{
			vgui::ipanel()->SetSize(root, viewActualWidth, viewActualHeight);
		}
		...
		// HLVR : Rendering the menu does not have to be done twice per frame...
		// Also, it does not either have to be done unless we actually draw the menu.
		// So this has got to move ! But right now I'd say fuck it

		ITexture* pMenuTexture = materials->FindTexture("_rt_menu", NULL, false);
		viewX = viewY = 0;
		viewActualWidth = pMenuTexture->GetActualWidth();
		viewActualHeight = pMenuTexture->GetActualHeight();
		...
		// constrain where VGUI can render to the view
		pRenderContext->SetRenderTarget(pMenuTexture);
		pRenderContext->Viewport(0, 0, viewActualWidth, viewActualHeight);
		// If drawing off-screen, force alpha for that pass
		if (pMenuTexture)
		{
			pRenderContext->OverrideAlphaWriteEnable(true, true);
		...
		if ( UseVR() )
		{
			...
			if( !bInFreezeCam && g_ClientVirtualReality.ShouldRenderHUDInWorld() )
			{
				...
				g_ClientVirtualReality.RenderHUDQuad(engine->IsPaused(), !engine->IsPaused() && !g_pClientMode->ShouldBlackoutAroundHUD());
				CleanupMain3DView( view );
			}
		}
...
void CViewRender::DrawScope(const CViewSetup &viewSet)
{
	...
	if (!localPlayer->GetRenderedWeaponModel())
		return;
	...
	if (UseVR())
	{
		C_BaseAnimating *vm = localPlayer->GetRenderedWeaponModel();
		if (vm)
		{
			QAngle tmp;
			int iAttachment = vm->LookupAttachment("scope");
			vm->GetAttachment(iAttachment, scopeView.origin, tmp);
			scopeView.angles = g_pHLVR->GetRightHandRotation();
		}
		else {
			g_pHLVR->GetEyeToRightHandOffset(scopeView.origin);
	...
	ITexture *pRenderTarget = NULL;// GetScopeTexture();	// Added for Anarchy Arcade
	...
	scopeView.x = 0;
	scopeView.y = 0;
	scopeView.fov = 1.f / hlvr_scopefov.GetFloat() * 7.f; 
...
void CSkyboxView::Draw()
{
	...
	{
		pRTColor = g_pHLVR->GetRenderTarget( (ISourceVirtualReality::VREye) (m_eStereoEye - 1), ISourceVirtualReality::RT_Color );
		pRTDepth = g_pHLVR->GetRenderTarget((ISourceVirtualReality::VREye) (m_eStereoEye - 1), ISourceVirtualReality::RT_Depth);
	}
...
CReflectiveGlassView* monoView;

// Added for Anarchy Arcade
void VectorsMatrix(const Vector &forward, const Vector &right, const Vector &up, matrix3x4_t& matrix)
{
...
// HLVR : TODO, FIX the mirrors!
bool CReflectiveGlassView::AdjustView( float flWaterHeight )
{
	ITexture *pTexture = GetWaterReflectionTexture();
	if (this->m_eStereoEye == STEREO_EYE_MONO)
		monoView = this;
	...
	/*Vector centerOfGlass = m_ReflectionPlane.normal * (m_ReflectionPlane.dist);
	Vector eyeOriginInGlass = origin - centerOfGlass;*/
	...
	matrix3x4_t eyeRotation;
	AngleMatrix(angles, eyeRotation);

	VectorTransform(Vector(1, 0, 0), eyeRotation, vecForward);
	VectorTransform(Vector(0, 1, 0), eyeRotation, vecRight);
	VectorTransform(Vector(0, 0, 1), eyeRotation, vecUp);
	...
	flDot = DotProduct(vecRight, m_ReflectionPlane.normal);
	VectorMA(vecRight, - 2.0f * flDot, m_ReflectionPlane.normal, vecRight);

	VectorsMatrix(vecForward, vecRight, vecUp, eyeRotation);

	VectorMA(origin, -2.0f * flDist, m_ReflectionPlane.normal, origin);

	MatrixAngles(eyeRotation, angles);
