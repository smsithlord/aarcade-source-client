...
public:
	...
	// HLVR :
	void			RenderVREyeToView(const CViewSetup &view, StereoEye_t eye, bool isMenuUp);
...
private:
	...
	// HLVR : Re-Implement Draw View Fade from CRender
	void			ViewDrawFade(const CViewSetup& viewSetup, byte *color, IMaterial* pFadeMaterial);
	...
	void			PerformScreenOverlay(const CViewSetup& viewSetup, int x, int y, int w, int h );
